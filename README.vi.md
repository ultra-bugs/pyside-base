...
```
project_root/
├── app/                  # 🏠 Logic ứng dụng (Userland)
│   ├── tasks/            # Các task cụ thể của ứng dụng
│   ├── windows/          # Cửa sổ và components Qt
│   ├── console/          # Wrapper CLI headless
│   └── extends/          # Mở rộng widget cấp app
├── core/                 # ⚙️ Base Framework (Không chỉnh sửa)
│   ├── BaseController.py
│   ├── taskSystem/
│   ├── WidgetManager.py
│   └── ...               # Các module core khác
├── tests/                # 🧪 Test ứng dụng
├── tests_core/           # 🔬 Test core framework
└── docs/                 # 📚 Tài liệu
```
...
class MyController(BaseController):
   ...
   # phương thức `onOpenBtnClick` trong handler sẽ được gọi
...
* **Subscriber (Handler)**: Đăng ký các phương thức `on<PascalCasedEventName>` — thêm/xóa listener mà không chạm vào Controller.
